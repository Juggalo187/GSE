...
function GSE.GetMacroIcon(classid, sequenceIndex)
    ...
    if GSE.isEmpty(sequence.Icon) and GSE.isEmpty(iconid) then
		...
		--[[
			...
			local specid,name1,specicon = GSE.GetCurrentSpecID()
...
function GSE.UpdateIcon(self, reset)
    ...
    if not executionseq or not executionseq[step] then
        GSE.PrintDebugMessage("No execution sequence found for " .. gsebutton .. " at step " .. step, "UpdateIcon")
        return
    end
    ...
    if not reset then
        ...
        if macroIndex and macroIndex ~= 0 then
            ...
            if iconTexture and _G[gsebutton.."Icon"] then
                _G[gsebutton.."Icon"]:SetTexture(iconTexture)
            end
        end
    end
    
    GSE.PrintDebugMessage("Updated icon for " .. gsebutton .. " at step " .. step .. (foundSpell and " with spell" or " with item"), "UpdateIcon")
end
