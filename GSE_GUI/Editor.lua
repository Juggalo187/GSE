...
editframe.SequenceName = GSE.GUIEditFrame.SequenceName or "" --skuly fix
editframe.Raid = 1
editframe.PVP = 1
editframe.Mythic = 1
editframe.Dungeon = 1
...
local myAceTimer1 = LibStub("AceTimer-3.0"):Embed(GSE)
local myAceTimer2 = LibStub("AceTimer-3.0"):Embed(GSE)
local frameTableUpdated = {}

local MyTimer1
local MyTimer2

local function MyTimer1Func()
MyTimer1 = nil
GSE.GUIEditFrame:SetStatusText(editframe.statusText)
end

local function MyTimer2Func()
MyTimer2 = nil
GSE.GUIEditFrame:SetStatusText(editframe.statusText)
end
...
editframe.frame:SetScript(
    ...
    function(self, width, height)
		...
		if editframe.Width > GetScreenWidth () then
            editframe.Width = GetScreenWidth() - 10
            editframe:SetWidth(editframe.Height)
        end
        if editframe.Height < 500 then
            editframe.Height = 500
            editframe:SetHeight(editframe.Height)
...
function GSE.GUIEditorPerformLayout(frame)
	...
	--DEFAULT_CHAT_FRAME:AddMessage("Child = "..child)
...
function GSE:GUIDrawMetadataEditor(container)
	...
	speciddropdown:SetValue(Statics.wotlkSpecIDList[editframe.Sequence.MetaData.SpecID])
...
if 	version ~= nil then
	...
	if not GSE.isEmpty(GSE.GUIEditFrame.SequenceName) then
    ...
    layoutcontainer:SetHeight(editframe.Height - 360)
    ...
    scrollcontainer:SetHeight(editframe.Height - 360)
    scrollcontainer:SetLayout("Fill") -- Important!
    ...
    if GSE.isEmpty(disableMove) then
        ...
        disableBlock:SetCallback(
            ...
            function(sel, object, value)
                ...
                if value == true then
                    highlightTexture:SetTexture(1, 0, 0, 0.15)
                else
                    highlightTexture:SetTexture(1, 0, 0, 0)
                end
            end
        )
